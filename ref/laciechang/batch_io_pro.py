...
class Add_retime_render(object):
    def __init__(
        ...
        direction,
        key_color="Pink",
        render_preset="test_preset_name",
        track=1,
        isVFXPullResoltion=False,
        ui_item=None,
    ):
        ...
        for i in self.all_clips:
            ...
            if clip_color == self.key_color:
                ...
                clipinfo = {
                    "Name": tClip.GetName(),
                    "In": tClip.GetStart(),
                    "Resolution": mpi.GetClipProperty("Resolution")
                    if mpi is not None
                    else "Null",
                }
                try:
        ...
        ui.Button(
            {
                ...
                "Flat": True,
                "IconSize": [10, 10],
                "Icon": ui.Icon({"File": temp_icon_path}),
            }
        )
    )
                ...
                [
                    ...
                    ui.HGroup(
                        ...
                        [
                            ...
                            ui.Button(
                                {"ID": RefreshPresets, "Text": "刷新", "Weight": 0}
                            ),
                        ],
                    ),
                    ui.Label(
                    ...
                    ui.HGroup(
                        ...
                        [
                            ui.CheckBox(
                                {
                                    "ID": EnableVFXPullMode,
                                    "Text": "按VFX Pull常见规范自动计算输出分辨率",
                                }
                            ),
                            ...
                            ui.Button(
                                {"ID": PreviewResolution, "Text": "分辨率预览", "Weight": 0}
                            ),
                        ],
                    ),
                    ui.Button(
                            ...
                            ui.Label(
                                {
                                    "ID": ProgressBar,
                                    "Visible": False,
                                    "StyleSheet": "max-height: 1px; background-color: rgb(102, 221, 39);border-width: 1px;border-style: solid;border-color: rgb(37,37,37);",
                                }
                            ),
                        ],
                    ),
...
dlg = disp.AddWindow(
    {
        ...
        "ID": "MyWin",
        "Geometry": [800, 500, 600, 400],  # position when starting  # width, height
    },
    window_01,
)
...
def _pop_resolution_status_window(ev):
    ...
    add = Add_retime_render(
        direction=itm[DefaultDirection].CurrentText, key_color=color, track=track
    )
    ...
    reso_win = disp.AddWindow(
        {
            ...
            "Geometry": [
                main_pos[1] + bt_pos[3] + 400,
                main_pos[2] + bt_pos[2] + 100,  # position when starting
                550,
                win_height,  # width, height
            ],
        },
        popup_status_window,
    )
...
def _run_add(ev):
    ...
    add = Add_retime_render(
        ui_item=itm,
        direction=itm[DefaultDirection].CurrentText,
        render_preset=preset,
        key_color=color,
        track=track,
        isVFXPullResoltion=enabled,
    )
    ...
    new_win = disp.AddWindow(
        {
            ...
            "Geometry": [
                main_pos[1] + bt_pos[3] + 20,
                main_pos[2] + bt_pos[2],  # position when starting
                120,
                500,  # width, height
            ],
        },
